<template>
    <div class="todo-container">
      <div class="todo-wrap">
        ...
        <TodoFooter :todos="todos" :deleteDoneTodos="deleteDoneTodos" :checkAll="checkAll"/>
      </div>
    </div>
</template>
...
export default {
  ...
  methods: {
    ...
    checkAll(all){
      all ? this.todos = this.todos.reduce((prev, todo) => prev.concat(todo.done ? {...todo} : {...todo, done:true}), [])
        : this.todos = this.todos.reduce((prev, todo) => prev.concat(!todo.done ? {...todo} : {...todo, done:false}), [])
    }
  },
};
</script>
